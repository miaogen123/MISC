        ...
        for(countsec=0;countsec<matrix;countsec++)
        {
              if(mine[countone][countsec]==20)
              {
                ...
                if(countone==1)
                {
                  if(countsec==1)
                  {
                    printf("\033[40;32;1mMN\033[40;30;0m");
                  }
                  else
                  {
                    MOVERIGHT(4*(countsec-1));
                    printf("\033[40;32;1mMN033[40;30;0m" );
                  }
                }
                else
                {
...
void Set_Blank(int row, int col, int mine[16][16])
{
  if(mine[row][col]>0)
  {
  ...
  else if(mine[row][col]==0)
  {
    ...
    if(row==0&&col==0)
    {
      Set_Blank(row+1, col, mine);
      Set_Blank(row, col+1, mine);
    }
...
void DisplayCursor(int mine[16][16], int row, int col)
{
    ...
    {
        Set_Blank(row-1, col-1, mine);
    }
}
int main(void)
{
    int matrix=16,countone=0, countsec=0,count=0;
    int row=0, col=0;
    char tempchar;
    char state;
